...
def expect(expr, msg=None):
    ...
    global _failed_expectations, _is_first_call, cnt
    call_api_function = sys._getframe(1).f_code.co_name
    caller = inspect.stack()[1][3]
    # 只是为了防止多个文件expect 且没有assert 暂时我们没有这个情况
    if _is_first_call.get(caller, True):
        if call_api_function != "wrapper":
            cnt += 1
            logger.info("{} times call".format(cnt))
        _is_first_call[caller] = False
        if cnt > 1:
            # 正常调用是不会到这一步的，除非有两个或者两个以上文件调用expect并且没有assert才会到这里
            _failed_expectations = []
...
def _log_failure(msg=None):
    ...
    _failed_expectations.append(Color.FAIL + 'Failed at "' + Color.ENDC + Color.OKBLUE + Color.UNDERLINE + '%s:%s' % (
        file_path, line) + Color.ENDC + Color.FAIL + '", in %s()%s\n%s' %
                                (funcname, ((
                                    '\n\t' + Color.BOLD + Color.UNDERLINE + 'ErrorMessage:' + Color.ENDC + Color.FAIL + '\t%s' % msg + Color.ENDC)),
                                 context))


def _report_failures():
    global _failed_expectations, cnt
    if _failed_expectations:
        (file_path, line, funcname) = inspect.stack()[2][1:4]
        report = [
            Color.WARNING + '\n\nassert_expectations() called at' + Color.ENDC,
            Color.UNDERLINE + Color.OKBLUE + '"%s:%s"' % (
                file_path, line) + Color.ENDC + Color.WARNING + ' in %s()\n' % (funcname),
